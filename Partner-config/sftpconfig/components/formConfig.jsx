...
export const formFields = {
  partnerId: {
    ...
    id: `${reduxFormId}-partnerId`,
    key: `${reduxFormId}-partnerId`,
    name: 'partnerId',
    label: <FormattedMessage {...messages.partnerId} />,
    required: true,
  },
  partnerName: {
    component: renderTextField,
...
const mergePropsForReduxFormDecorator = (dispatchProps, ownProps) => {
  ...
  const validate = validator(formFields, (initialErrors, values) => {
    ...
    if (partnerLogo && partnerLogo.length < 2) {
      errors.name = <FormattedMessage {...messages.lengthError} />
    }
    if (vendor && vendor.length < 2) {
      errors.name = <FormattedMessage {...messages.lengthError} />
    }
    if (contentScope === undefined || contentScope === 'select') {
      errors.sftpSource = <FormattedMessage {...messages.sftpSourceError} />
    }
    if (curationPermission === undefined || curationPermission === 'select') {
      errors.sftpSource = <FormattedMessage {...messages.sftpSourceError} />
    }
    if (launchSource === undefined || launchSource === 'select') {
      errors.sftpSource = <FormattedMessage {...messages.sftpSourceError} />
    }
    if (launchTarget === undefined || launchTarget === 'select') {
      errors.sftpSource = <FormattedMessage {...messages.sftpSourceError} />
    }
    if (trackingMethod === undefined || trackingMethod === 'select') {
      errors.sftpSource = <FormattedMessage {...messages.sftpSourceError} />
    }
    if (status === undefined || status === 'select') {
      errors.sftpSource = <FormattedMessage {...messages.sftpSourceError} />
    }
    if (businessRelationship === undefined || businessRelationship === 'select') {
      errors.sftpSource = <FormattedMessage {...messages.sftpSourceError} />
    }
    if (businessModel === undefined || businessModel === 'select') {
      errors.sftpSource = <FormattedMessage {...messages.sftpSourceError} />
    }
    if (dataStorageRegion === undefined || dataStorageRegion === 'select') {
      errors.sftpSource = <FormattedMessage {...messages.sftpSourceError} />
    }
    return errors
  })
